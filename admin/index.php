...
if (isset($_GET['token']) || isset($_GET['img'])) {
    ...
    foreach ($files as $f) {
        if ($f['token'] === $token) {
            if ($f['expires'] < time()) { http_response_code(410); echo 'This link has expired.'; exit; }
...
if ($authed) {
    ...
    if (isset($_POST['action'])) {
        ...
        if ($action === 'save_snip') {
            ...
            foreach ($snips as &$s) {
                if ($s['id'] === $id) {
                    $s['label'] = $label; $s['text'] = $text; $s['updated'] = time();
                    $found = true; break;
                }
            }
            unset($s);
            if (!$found) $snips[] = ['id' => $id, 'label' => $label, 'text' => $text, 'created' => time(), 'updated' => time()];
        ...
        if ($action === 'del_file') {
            $token = $_POST['token'];
            foreach ($files as $f) {
                if ($f['token'] === $token) { @unlink($uploadDir . '/' . $token . '_' . $f['name']); break; }
            }
        ...
        if ($action === 'get_data') {
            ...
            if (count($clean) !== count($files)) {
                foreach ($files as $f) {
                    if ($f['expires'] <= $now) @unlink($uploadDir . '/' . $f['token'] . '_' . $f['name']);
                }
...
<head>
  ...
  <style>
    ...
    .lock-title em { font-style: italic; }
    .lock-field {
      display: flex;
      flex-direction: column;
      gap: 8px;
    ...
    .lock-input:focus { border-color: var(--border3); }
    .lock-error {
      font-size: 11px;
      color: #f87171;
      display: none;
    ...
    .lock-submit:hover { background: rgba(255,255,255,0.11); border-color: var(--border3); }
    ...
    .admin-title em { font-style: italic; }
    .logout-btn {
    ...
    .logout-btn:hover { border-color: var(--border2); color: var(--text2); }
    ...
    .admin-tab.active { background: rgba(255,255,255,0.08); color: var(--text); }
    ...
    .upload-zone-text strong { color: var(--text2); font-weight: 500; }
    ...
    .expire-btn:hover { border-color: var(--border2); color: var(--text2); }
    .expire-btn.active { background: rgba(255,255,255,0.08); border-color: var(--border2); color: var(--text); }
    ...
    .item-row-icon svg { width: 16px; height: 16px; stroke: var(--text3); fill: none; stroke-width: 1.5; stroke-linecap: round; stroke-linejoin: round; }
    .item-thumb {
      width: 36px;
      height: 36px;
      border-radius: var(--r-sm);
    ...
    .item-meta { flex: 1; min-width: 0; display: flex; flex-direction: column; gap: 4px; }
    .item-name { font-size: 13px; color: var(--text); font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .item-sub { font-size: 11px; color: var(--text4); display: flex; gap: 10px; flex-wrap: wrap; }
    .item-url {
      font-size: 11px;
      color: var(--text3);
      font-family: var(--mono);
    ...
    .item-url:hover { border-color: var(--border2); color: var(--text2); }
    ...
    .icon-btn.danger:hover { border-color: rgba(248,113,113,0.4); color: #f87171; }
    ...
    .snip-form-row { display: flex; gap: 10px; align-items: center; }
    .snip-label-input {
      flex: 1;
      background: var(--bg3);
      border: 1px solid var(--border);
    ...
    .snip-label-input:focus { border-color: var(--border3); }
    .snip-label-input::placeholder { color: var(--text4); }
    .snip-textarea {
      width: 100%;
      background: var(--bg3);
      border: 1px solid var(--border);
    ...
    .snip-textarea:focus { border-color: var(--border3); }
    .snip-textarea::placeholder { color: var(--text4); }
    .snip-save-btn {
      padding: 8px 18px;
      background: rgba(255,255,255,0.07);
      border: 1px solid var(--border2);
    ...
    .snip-save-btn:hover { background: rgba(255,255,255,0.11); }
...
<div class="admin-wrap">
  ...
  <div class="admin-section active" id="tab-files">
    ...
    <div class="upload-progress" id="file-progress">Uploading…</div>
  ...
  <div class="admin-section" id="tab-snippets">
    <div class="snip-form" id="snip-form">
      ...
      <div class="snip-form-row">
        ...
        <button class="snip-save-btn" onclick="saveSnip()">Save snippet</button>
      </div>
    </div>
  ...
  <div class="admin-section" id="tab-images">
    ...
    <div class="upload-progress" id="img-progress">Uploading…</div>
...
function bindUploadZone(zoneId, inputId, type) {
  ...
  input.addEventListener('change', () => {
    [...input.files].forEach(f => uploadFile(f, type));
    input.value = '';
  });
  ...
  zone.addEventListener('drop', e => {
    ...
    [...e.dataTransfer.files].forEach(f => uploadFile(f, type));
  });
}
...
function renderFiles(files) {
  ...
  list.slice().reverse().forEach(f => {
    ...
    row.innerHTML = `
      <div class="item-row-icon"><svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/></svg></div>
      <div class="item-meta">
        <div class="item-name">${f.name}</div>
        <div class="item-sub"><span>${fmtSize(f.size)}</span><span>${timeLeft(f.expires)}</span></div>
        <div class="item-url" onclick="copyAndToast('${url}')" title="Click to copy">${url}</div>
      </div>
...
function renderImages(files) {
  ...
  if (!list.length) { el.innerHTML = '<div class="empty-state">No hosted images</div>'; return; }
  el.innerHTML = '';
  list.slice().reverse().forEach(f => {
    const url = BASE + '/admin/?img=' + f.token;
    const row = document.createElement('div');
    row.className = 'item-row';
    row.innerHTML = `
      <img class="item-thumb" src="${url}" alt="${f.name}" loading="lazy">
      <div class="item-meta">
        <div class="item-name">${f.name}</div>
        <div class="item-sub"><span>${fmtSize(f.size)}</span><span>${timeLeft(f.expires)}</span></div>
        <div class="item-url" onclick="copyAndToast('${url}')" title="Click to copy">${url}</div>
      ...
      <div class="item-actions">
        <button class="icon-btn" title="Copy URL" onclick="copyAndToast('${url}')">
          <svg viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
        </button>
        <button class="icon-btn danger" title="Delete" onclick="delFile('${f.token}')">
          <svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6M14 11v6"/></svg>
